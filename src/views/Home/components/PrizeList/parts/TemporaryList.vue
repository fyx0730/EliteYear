...
<template>
  <div class="h-24 w-[22rem]" :class="temporaryPrize.isShow ? 'current-prize' : ''">
    <div class="relative flex flex-row items-center justify-between w-full h-full shadow-xl card bg-base-100">
      ...
      />
      <figure class="w-[4.5rem] h-[4.5rem] rounded-xl">
      ...
      <div class="items-center p-0 text-center card-body">
        <div class="tooltip tooltip-left" :data-tip="temporaryPrize.name">
          <h2 class="p-0 m-0 overflow-hidden w-32 card-title whitespace-nowrap text-ellipsis text-sm">
            {{ temporaryPrize.name }}
          </h2>
        </div>
        <div class="relative flex justify-center">
          ...
          />
          <p class="absolute left-1/2 top-[45%] -translate-x-1/2 -translate-y-1/2 z-40 p-0 m-0 text-gray-900 font-semibold text-xs drop-shadow-sm pointer-events-none">
